...
def execute_schema_processors(schema):
    for cmd in frappe.get_hooks("graphql_schema_processors"):
        try:
            ...
            # Log but don't fail - some processors may not be available
            # Use print instead of log_error to avoid triggering doc_events hooks
            if frappe.conf.developer_mode:
                print(f"Warning: Failed to load GraphQL schema processor {cmd}: {str(e)}")
